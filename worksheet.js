  ...
  $(document).ready(function () {
    ...
    tableau.extensions.initializeAsync().then(function () {
      ...
      dashboard.worksheets.forEach(function (worksheet) {
        ...
        underlyingTablesPromises.push(worksheet.getUnderlyingTablesAsync());
        worksheetsArr.push(worksheet);
      });

      Promise.all(underlyingTablesPromises).then( (fetchResults) => {
        
        fetchResults.forEach(function (logicalTables,i) {

          underlyingTablesDataPromises.push(worksheetsArr[i].getUnderlyingTableDataAsync(logicalTables[0].id))
        ...
        Promise.all(underlyingTablesDataPromises).then( (dataTables) => {
            ...
            dataTables.forEach( (d,i) => {
              ...
              data[worksheetsArr[i].name] = dataTables[i]
            })
